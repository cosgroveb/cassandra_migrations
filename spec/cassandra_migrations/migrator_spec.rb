...
describe CassandraMigrations::Migrator do
  ...
  it "requires the class name of the migration to match the file name" do
    allow(described_class).to receive(:read_current_version).and_return(0)
    allow(described_class).to receive(:get_all_migration_names).and_return(["#{Dir.pwd}/spec/fixtures/db/cassandra_migrate/invalid/12345_non_matching_name.rb"])

    expect { described_class.up_to_latest! }.to raise_exception(CassandraMigrations::Errors::MigrationNamingError, "Migration file names must match the class name in the migration".red)
  end
end
